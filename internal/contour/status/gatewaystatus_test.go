...
import (
	...
	"epic-gateway.org/puregw/internal/contour/k8s"
...
func TestGatewayAddCondition(t *testing.T) {
	...
	got := gatewayUpdate.AddCondition(gatewayapi.GatewayConditionScheduled, metav1.ConditionTrue, ReasonValidGateway,
		"Valid Gateway")
...
func TestGatewaySetListenerSupportedKinds(t *testing.T) {
	...
	gsu.SetListenerSupportedKinds("http", []gatewayapi.Kind{"HTTPRoute"})
	gsu.SetListenerSupportedKinds("https", []gatewayapi.Kind{"HTTPRoute", "TLSRoute"})
	...
	assert.ElementsMatch(t,
		[]gatewayapi.RouteGroupKind{
			{Group: epicgatewayapi.GroupPtr(gatewayapi.GroupName), Kind: "HTTPRoute"},
		},
	...
	assert.ElementsMatch(t,
		[]gatewayapi.RouteGroupKind{
			{Group: epicgatewayapi.GroupPtr(gatewayapi.GroupName), Kind: "HTTPRoute"},
			{Group: epicgatewayapi.GroupPtr(gatewayapi.GroupName), Kind: "TLSRoute"},
		},
...
func TestGatewayMutate(t *testing.T) {
	...
	gsu.ListenerStatus = map[string]*gatewayapi.ListenerStatus{
		"http": {
			Name:           "http",
			AttachedRoutes: 7,
			SupportedKinds: []gatewayapi.RouteGroupKind{
				{
					...
					Kind:  gatewayapi.Kind("FooRoute"),
				},
				{
					Group: epicgatewayapi.GroupPtr(gatewayapi.GroupName),
					Kind:  gatewayapi.Kind("BarRoute"),
				},
			},
	...
	var want []gatewayapi.ListenerStatus
...
func TestGatewayAddListenerCondition(t *testing.T) {
	...
	res := gsu.AddListenerCondition("listener-1", gatewayapi.ListenerConditionReady, metav1.ConditionFalse, gatewayapi.ListenerReasonInvalid, "message 1")
	assert.Len(t, gsu.ListenerStatus["listener-1"].Conditions, 1)
	assert.Equal(t, string(gatewayapi.ListenerConditionReady), res.Type)
	assert.Equal(t, metav1.ConditionFalse, res.Status)
	assert.Equal(t, string(gatewayapi.ListenerReasonInvalid), res.Reason)
	assert.Equal(t, "message 1", res.Message)

	// second condition (different type) for listener-1
	res = gsu.AddListenerCondition("listener-1", gatewayapi.ListenerConditionDetached, metav1.ConditionTrue, gatewayapi.ListenerReasonUnsupportedProtocol, "message 2")
	assert.Len(t, gsu.ListenerStatus["listener-1"].Conditions, 2)
	assert.Equal(t, string(gatewayapi.ListenerConditionDetached), res.Type)
	assert.Equal(t, metav1.ConditionTrue, res.Status)
	assert.Equal(t, string(gatewayapi.ListenerReasonUnsupportedProtocol), res.Reason)
	assert.Equal(t, "message 2", res.Message)

	// first condition for listener-2
	res = gsu.AddListenerCondition("listener-2", gatewayapi.ListenerConditionReady, metav1.ConditionFalse, gatewayapi.ListenerReasonInvalid, "message 3")
	...
	res = gsu.AddListenerCondition("listener-1", gatewayapi.ListenerConditionDetached, metav1.ConditionTrue, gatewayapi.ListenerReasonUnsupportedProtocol, "message 4")
	assert.Len(t, gsu.ListenerStatus["listener-1"].Conditions, 2)
	assert.Equal(t, string(gatewayapi.ListenerConditionDetached), res.Type)
	assert.Equal(t, metav1.ConditionTrue, res.Status)
	assert.Equal(t, string(gatewayapi.ListenerReasonUnsupportedProtocol), res.Reason)
...
func TestGetGatewayConditions(t *testing.T) {
	tests := map[string]struct {
		conditions []metav1.Condition
		want       map[gatewayapi.GatewayConditionType]metav1.Condition
	}{
		...
		"one gateway condition": {
			...
			want: map[gatewayapi.GatewayConditionType]metav1.Condition{
				gatewayapi.GatewayConditionReady: {Type: string(gatewayapi.GatewayConditionReady)},
			},
		},
		"multiple gateway conditions": {
			...
			want: map[gatewayapi.GatewayConditionType]metav1.Condition{
				gatewayapi.GatewayConditionReady:     {Type: string(gatewayapi.GatewayConditionReady)},
				gatewayapi.GatewayConditionScheduled: {Type: string(gatewayapi.GatewayConditionScheduled)},
			},
		},
	}
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			got := getGatewayConditions(&gatewayapi.GatewayStatus{Conditions: tc.conditions})
